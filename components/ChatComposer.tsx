...
export default function ChatComposer({
  ...
  const startRecording = async () => {
    try {
      ...
      // Start live transcription simulation (replace with real service)
  ...
  const stopRecording = async () => {
    try {
      ...
      if (liveTranscription.trim()) {
        const newText = currentInput ? `${currentInput} ${liveTranscription}` : liveTranscription;
  ...
  // Live transcription simulation - replace with real service like Google Speech-to-Text
  const startLiveTranscription = () => {
    ...
    const addWord = () => {
      if (currentIndex < words.length && isRecording) {
        setLiveTranscription(prev => prev ? `${prev} ${words[currentIndex]}` : words[currentIndex]);
        currentIndex++;
