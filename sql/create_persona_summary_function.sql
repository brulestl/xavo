-- Create a Supabase RPC function to generate rich persona summaries
-- This function will be called from the Expo app after quiz completion

CREATE OR REPLACE FUNCTION generate_persona_summary(
...
BEGIN
  ...
  persona_summary := format(
    'As a %s leading %s in a %s-person firm, you communicate %s, tackle conflict by %s, and decide via %s. You face challenges like %s. Your trait scores are: Assertiveness %s%%, Strategic %s%%, Adaptability %s%%, Empathy %s%%, Conscientiousness %s%%, Integrity %s%%.',
    role,
    primary_function,
    company_size,
    communication_style,
  ...
  IF length(persona_summary) > 500 THEN
    persona_summary := left(persona_summary, 497) || '...';
  END IF;

  -- Update the user_personalization table with the generated summary
  UPDATE user_personalization 
  SET 
    quiz_summary = persona_summary,
    updated_at = NOW()
  WHERE user_personalization.user_id = target_user_id;

  -- Return the summary
  RETURN json_build_object('summary', persona_summary);

EXCEPTION
  WHEN OTHERS THEN
    -- Generate fallback summary on any error
    fallback_summary := format(
      'As a %s leading %s in a %s-person firm, you communicate directly and clearly, tackle conflict by addressing issues head-on, and decide via data-driven analysis. You face challenges like %s. Your trait scores are: Assertiveness 50%%, Strategic 50%%, Adaptability 50%%, Empathy 50%%, Conscientiousness 50%%, Integrity 50%%.',
