-- User Personalization Schema (Nuclear Option)
-- This file uses the most aggressive approach to clean up function conflicts
-- Uses system catalog queries to drop ALL function variations
...
-- NUCLEAR OPTION: Drop ALL functions with these names using system catalog
DO $$
DECLARE
    func_record RECORD;
    drop_statement TEXT;
BEGIN
    -- Drop all functions named fn_insert_or_update_personalization
    FOR func_record IN 
        SELECT 
            p.proname,
        ...
        WHERE p.proname = 'fn_insert_or_update_personalization'
    ...
    -- Drop all functions named fn_calculate_personality_scores
    FOR func_record IN 
        SELECT 
            p.proname,
            pg_get_function_identity_arguments(p.oid) as args
        FROM pg_proc p
        JOIN pg_namespace n ON p.pronamespace = n.oid
        WHERE p.proname = 'fn_calculate_personality_scores'
        AND n.nspname = current_schema()
    LOOP
        drop_statement := 'DROP FUNCTION IF EXISTS ' || func_record.proname || '(' || func_record.args || ')';
        RAISE NOTICE 'Executing: %', drop_statement;
    ...
    RAISE NOTICE 'All function variations dropped using system catalog method';
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Error during function cleanup: %', SQLERRM;
END $$;
...
DO $$
BEGIN
    RAISE NOTICE '🎉 User personalization schema setup completed successfully with nuclear cleanup! 🎉';
END $$; 